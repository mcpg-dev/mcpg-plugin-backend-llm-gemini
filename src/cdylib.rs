...
//! three Gemini plugin types onto the sync FFI trait the cdylib vtable
//! expects ([`SyncBackendPlugin`]). Each wrapper owns a private
//! multi-thread runtime and `block_on`s the async logic; the make-time
//! [`HostHandle`] is turned into an `Arc<dyn BackendHost>` (via
//! [`HostHandleBackendHost`]) that is passed to `register_profile`, and
//! — for the chat variant that exposes `set_host_handle` — also
//! installed on the inner plugin for unified observability.
//!
//! Mirrors the proven nats / kafka pilots
//! (`libs/plugins/backend/{nats,kafka}`) and the sibling OpenAI
//! conversion (`libs/plugins/backend/llms/openai`). Deviations from the
...
//! - Only `GeminiChatCdylib` calls `inner.set_host_handle(...)` — only
//!   the chat type exposes it.
...
use crate::embedding_plugin::GeminiEmbeddingPlugin;
use crate::image_plugin::GeminiImagePlugin;
use crate::plugin::GeminiChatPlugin;
...
fn build_bridge_runtime(thread_name: &str) -> tokio::runtime::Runtime {
    tokio::runtime::Builder::new_multi_thread()
        ...
        .enable_all()
        .build()
        .unwrap_or_else(|e| panic!("gemini cdylib: tokio runtime init failed: {e}"))
}
...
/// chat variant that exposes the method; omit it for the two that do
/// not. The host-handle is always wrapped into an `Arc<dyn BackendHost>`
/// and passed to `register_profile` regardless.
    ...
    (
        ...
        impl $wrapper {
            ...
            pub fn from_host_config(_config_json: &str, host: HostHandle) -> Self {
                let inner = <$inner>::new();
                $(
                    // Only emitted for the chat variant ($shh just
...
// --- chat (the variant that exposes `set_host_handle`) ---------------
backend_cdylib_wrapper! {
    /// `SyncBackendPlugin` bridge over [`GeminiChatPlugin`].
    GeminiChatCdylib, GeminiChatPlugin, "mcpg-llm-gemini-chat", set_host_handle = true
}

// --- embedding -------------------------------------------------------
backend_cdylib_wrapper! {
    /// `SyncBackendPlugin` bridge over [`GeminiEmbeddingPlugin`].
    GeminiEmbeddingCdylib, GeminiEmbeddingPlugin, "mcpg-llm-gemini-embedding"
}

// --- image -----------------------------------------------------------
backend_cdylib_wrapper! {
    /// `SyncBackendPlugin` bridge over [`GeminiImagePlugin`].
    GeminiImageCdylib, GeminiImagePlugin, "mcpg-llm-gemini-image"
}

// cdylib export — all three entities under `dev.mcpg.backend.llm.gemini`.
// Each entity reports its real kind (gemini.chat, gemini.embedding,
// gemini.image) through its `manifest()` slot at runtime, so the host
// dispatches each binding to the correct wrapper. Distinct `inner_name`
// slugs keep the registry aliases (`{plugin_id}:{inner_name}`) unique;
// the chat type gets the bare `""` alias.
mcpg_plugin_sdk::declare_plugin! {
    ...
    entities: [
        backend as chat {
            inner_name: "",
            plugin_type: GeminiChatCdylib,
            factory: |cfg, host: ::mcpg_plugin_sdk::HostHandle|
                GeminiChatCdylib::from_host_config(cfg, host),
        },
        backend as embedding {
            inner_name: "embedding",
            plugin_type: GeminiEmbeddingCdylib,
            factory: |cfg, host: ::mcpg_plugin_sdk::HostHandle|
                GeminiEmbeddingCdylib::from_host_config(cfg, host),
        },
        backend as image {
            inner_name: "image",
            plugin_type: GeminiImageCdylib,
            factory: |cfg, host: ::mcpg_plugin_sdk::HostHandle|
                GeminiImageCdylib::from_host_config(cfg, host),
        },
    ],
}
